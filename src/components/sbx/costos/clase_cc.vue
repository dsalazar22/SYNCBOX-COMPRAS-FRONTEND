<template>
    <div>
        ...
        <h4 class="font-weight-bold py-3 mb-4">Clase Centros de Costos</h4>
        ...
        <b-modal id="modals-top" v-model="showFormulario"  class="modal-center" size="md">
            ...
            <fieldset class="demo-vertical-spacing-sm">
                <b-form-group :feedback="objState.message" label="Codigo de la Clase">
                    <b-input placeholder="Codigo de la Clase" v-model.trim="objInfo.code"/>
                </b-form-group>
        ...
        ///Eventos CRUD
            submit: function(){
                if(this.validate_content()){
                    infocosts.claseCC(this.objInfo, this.objInfo.tipos_de_cuenta_id === 0 ? "insert" : "update").then(data => {
                        if (data.status === 202){
            ...
            deleteRecord: function(){
                infocosts.claseCC(this.objInfo, "delete").then(data => {
                    if (data.status === 202){
