<template>
    <div>
        ...
        <h4 class="font-weight-bold py-3 mb-4">Presupuesto  <span class="font-weight-light">Centros de Costos</span></h4>
        <div class="w-100 text-right"></div>
            ...
            <div v-if="selectTab=='ver_registros'">
                ...
                <b-table responsive small show-empty :items="tblOriginal" :fields="columns"
                    :filter="filter" @filtered="onFiltered"  :style="{'max-height': (windowHeight-280).toString()+'px'}" 
...
<script>
    ...
    export default {
        ...
        watch: {
            'tagsCCostos':function(values){
                
                this.tblFilter = this.tableData
                ...
                for (let index = 0; index < values.length; index++) {
                    ...
                    if(element.toLowerCase() == "cc"){
                        columnFilter = element.toLowerCase()
                        if(entry){
                            
                            this.tblFilter = JSON.parse(JSON.stringify(tablaResult))
                            tablaResult=[]
                            entry=false
                        }else{
                            entry = true
                        }
                    }else if(element.toLowerCase() == "co"){
                        columnFilter = element.toLowerCase()
                        if(entry){
                            this.tblFilter = JSON.parse(JSON.stringify(tablaResult))
                            tablaResult=[]
                        ...
                        if(element == ":"){
                            ...
                            if(counterActive){
                                maxNumericValue = parseInt(element)
                            }else{
                                lastNumericValue =parseInt(element)
                            }
                        }
                    }
                    ...
                    if(lastNumericValue != 0){
                        ...
                        if(!counterActive){
                            for (let index2 = 0; index2 < this.tblFilter.length; index2++) {
                                const el = this.tblFilter[index2];
                                if(columnFilter == 'cc'){
                                    if(el.ccostoscode==lastNumericValue){
                                        tablaResult.push(el)
                                    }
                                } else if(columnFilter == 'co'){
                                    if(el.ccontablecode==lastNumericValue){
                                        tablaResult.push(el)
                                    }
                                }
                            }
                        } else if(counterActive && maxNumericValue != 0){
                            
                            for (let index2 = 0; index2 < this.tblFilter.length; index2++) {
                                const el = this.tblFilter[index2];
                                if(columnFilter == 'cc'){
                                    if(el.ccostoscode>lastNumericValue && el.ccostoscode<maxNumericValue){
                                        tablaResult.push(el)
                                    }
                                }else if(columnFilter == 'co'){
                                    if(el.ccontablecode>lastNumericValue  && el.ccontablecode<maxNumericValue){
                                        tablaResult.push(el)
                                    }
                                }
                            }
                ...
                if(tablaResult.length>0){
                    this.tblOriginal = JSON.parse(JSON.stringify(tablaResult))
                    let obj = {
                        cc_description:'',
                        cc_id:0,
                        ccontablecode:'',
        ...
        data: () => ({
            ...
            /// Columnas de las tablas
                columns: [
                    {key:'planta', sortable:true, label:"Planta"},
                    {key:'ccostoscode', sortable:true, label:"CCostos Codigo"},
                    {key:'ccosctoddesc', sortable:true, label:"CCostos Descrip"},
                    {key:'ccontablecode', sortable:true, label:"CContable Codigo"},
                    {key:'ccontabledesc', sortable:true, label:"CContable Desc"},
        ...
        methods:{
            ...
            ejecutarDescartePPTO(){
                infocosts.ctrlpresupuesto(this.datosCompletos[0].presupuesto_id, "descartarPPTO").then(data =>{
                    if (data.status === 202){
            ...
            activatePresupuesto(){
                 infocosts.ctrlpresupuesto(this.datosCompletos[0].presupuesto_id, "activateppto").then(data =>{
                    if (data.status === 202){
            ...
            cancelPresupuesto(){
                infocosts.ctrlpresupuesto(this.datosCompletos[0].presupuesto_id, "cancelppto").then(data =>{
                    if (data.status === 202){
            ...
            guardarArchivoDB(){
                infocosts.uploadFile('savepresupuestocc'+'|'+this.pp_year, this.file1).then(data =>{
                    if (data.status === 202){
                        this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                        this.disabled_loadfile=false
                        this.datosCompletos=null
            ...
            openCC(item){
                ...
                this.showFormularioCC=true; 
                this.objInfo.code = item.ccostoscode.trim(); 
                this.objInfo.description = item.ccosctoddesc.trim();
                this.objInfo.inactive = true
            },
            ...
            openPUC(item){
                ...
                this.objInfo.code = item.ccontablecode.trim(); 
                this.objInfo.description = item.ccontabledesc.trim();
                this.objInfo.inactive = true
                this.showFormularioPUC=true
            },
             ...
             submitPUC: function(){
                ...
                this.objInfo.tipos_de_cuenta_id = this.selectedTipoCuenta.tipos_de_cuenta_id
                    infocosts.puc(this.objInfo, "insert").then(data => {
                        if (data.status === 202){
                            this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                        }else{
                            this.showCustom('bg-success text-white', "Guardar registro","¡El registro se ha Guardado con Éxito!")
                            this.showFormularioPUC=false
                            this.cargarArchivo()
                        }
                    })
            ...
            submitCC: function(){
                // if(this.validate_content()){
                    ...
                    infocosts.centroscostos(this.objInfo, "insert").then(data => {
                        if (data.status === 202){
                            this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                        }else{
                            // if(this.objInfo.cc_id === 0)
                            this.showCustom('bg-success text-white', "Guardar registro","¡El registro se ha Guardado con Éxito!")
                            this.showFormularioCC=false
                            this.cargarArchivo()
                        }
                    })
                    .catch(function (error) {
        ...
        ///Eventos CRUD
            ...
            showData: function(){
                infocosts.ctrlpresupuesto([], "select").then(data =>{
                    if (data.data === ""){
                        this.disabled_loadfile=false
                        this.datosCompletos=null
                        // this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
        ...
        ///Preracion de datos para el borrado y el modificado desde el evento de la tabla
            ...
            cargarArchivo(){
                ...
                this.title_import=''
                this.infook=false
                infocosts.uploadFile('check_pptocontable', this.file1).then(data =>{
                    if(data.data != ""){
                        this.infook = false
                        this.title_import = 'ERROR AL SUBIR EL ARCHIVO / REVISE DUPLICADOS, DATOS EN BLANCO, CUENTAS SIN CREAR'
                        this.tableData=data.data
                    }else{
                        infocosts.uploadFile('pptocontable', this.file1).then(data =>{
                            let errors = []
                            for (let index = 0; index < data.data.length; index++) {
                                const element = data.data[index];
                                if(element.cc_id == null || element.puc_id == null){
                                    errors.push(element)
                                }
                            }
