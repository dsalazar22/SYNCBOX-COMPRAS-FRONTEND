<template>
    <div>
        ...
        <h4 class="font-weight-bold py-3 mb-4">Sociedades Financieras</h4>
        ...
        <b-modal id="modals-top" v-model="showFormulario"  class="modal-center" size="md">
            ...
            <fieldset class="demo-vertical-spacing-sm">
                <b-form-group :feedback="objState.message" label="NIT">
                    <b-input placeholder="NIT" v-model.trim="objInfo.nit"/>
                </b-form-group>
                <b-form-group :feedback="objState.message" label="Nombre">
                    <b-input placeholder="Nombre" v-model.trim="objInfo.name" />
                </b-form-group>
                <b-form-group :feedback="objState.message" label="Moneda">
                    <b-input placeholder="Moneda" v-model.trim="objInfo.currency" />
                </b-form-group>
            </fieldset>
        ...
        ///Eventos CRUD
            submit: function(){
                if(this.validate_content()){
                    infocosts.sf(this.objInfo, this.objInfo.sf_id === 0 ? "insert" : "update").then(data => {
                        if (data.status === 202){
                            this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                        }else{
                            if(this.objInfo.sf_id === 0)
            ...
            deleteRecord: function(){
                infocosts.sf(this.objInfo, "delete").then(data => {
                    if (data.status === 202){
