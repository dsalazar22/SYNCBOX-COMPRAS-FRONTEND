<template>
    <div>
        ...
        <h4 class="font-weight-bold py-3 mb-4">Código de Defectos</h4>
        ...
        <div class="col">
           ...
           <b-row>
               ...
               <b-col md="2" class="d-flex justify-content-center my-1">
                   <b-btn variant="outline-success icon-btn" class="btn-md" v-if="check_access('floor_scrap', 'c')"  v-b-modal.modals-default @click.prevent="addData();objState={}"><i class="ion ion-md-add"></i></b-btn>&nbsp;
           ...
           <b-table small show-empty stacked="md" :items="tableData" :fields="columns" :current-page="currentPage" :per-page="perPage" 
               ...
               <template v-slot:cell(edit)="row">
                   <div class="text-left">
                       <b-btn variant="outline-success borderless icon-btn" class="btn-xs" v-if="check_access('floor_scrap', 'c')"  @click.prevent="data_change(row.item,'edit')"><i class="ion ion-md-create"></i></b-btn>
           ...
           <div class="d-flex justify-content-between">
               ...
               <div class="text-muted">
                   <small> Total Defectuosas: {{this.totalRows}} </small>
               </div>
           </div>
        </div> 
        ...
        <sweet-modal icon="error" title="Borrar registro" ref="errorWithButtonAlert">
            ...
            <template slot="button"><b-btn v-if="check_access('floor_scrap', 'd')" @click="deleteRecord()" variant="danger" class="mr-2">Eliminar</b-btn></template>
        ...
        <b-modal id="modals-top" v-model="showFormulario"  class="modal-center" size="md">
            <div slot="modal-title">
                Administrar <span class="font-weight-light">Tipos de Defectuosas</span><br>
            ...
            <fieldset class="demo-vertical-spacing-sm">
                <b-form-group :feedback="objState.message" label="Código del Defecto">
                    <b-input placeholder="Código del Defecto" v-model.trim="objInfo.code" :state="objState.code!=''" v-on:input="objState.code = 'valid'; objState.code= objInfo.code==''?'invalid':'valid'"/>
                </b-form-group>
                <b-form-group :feedback="objState.message" label="Descripción del Defecto">
                    <b-input placeholder="Descripción del Defecto" v-model.trim="objInfo.description" :state="objState.description!=''" v-on:input="objState.description = 'valid'; objState.description= objInfo.description==''?'invalid':'valid'"/>
                </b-form-group>
                <b-form-group :feedback="objState.family">
                    <b-form-group label="Adicione la Familia" :feedback="objState.family" >
                    <multiselect
                        v-model="objInfo.families"
                        label="description"
                ...
                <b-form-row>
                    <b-form-group class="col-md-12" label="Comentario" >
                        <b-textarea rows="5" max-rows="5"  v-model="objInfo.comment"/>
                    </b-form-group>
                </b-form-row>
            </fieldset>
            <div slot="modal-footer" class="w-100 text-right">
                <div class="d-flex justify-content-between">
                    ...
                    <div>
                        ...
                        <b-btn :variant="variantSaveButton" v-if="check_access('floor_scrap', 'c')" @click="submit()">Guardar</b-btn>
                    </div>
                </div>
            </div>
        </b-modal>
    ...
    export default {
        ...
        data: () => ({
            ...
            /// Columnas de las tablas
                columns: [
                    ...
                    {key:'families_details', label:'Familia'},
        ...
        ///Eventos CRUD
            submit: function(){
                ...
                if(this.validate_content()){
                    infomaster.scrapcode(this.objInfo, this.objInfo.scrap_code_id === undefined ? "insert" : "update").then(data => {
                        if (data.status === 202){
                            this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                        }else{
                            if(this.objInfo.scrap_code_id === undefined)
            ...
            deleteRecord: function(){
                infomaster.scrapcode(this.objInfo, "delete").then(data => {
                    if (data.status === 202){
            ...
            showData: function(){
                infomaster.families("prd","select").then(data => {
                    this.tableFamily = data.data
                })

                infomaster.scrapcode([],"select").then(data => {
                    if(data.data != ''){
        ...
        ///Preracion de datos para el borrado y el modificado desde el evento de la tabla
            data_change :function(row,action){
                ...
                this.objInfo= JSON.parse(JSON.stringify(row))
                // this.objInfo.families = JSON.parse(JSON.stringify(row.families_selected))
                // console.log(row.families_selected)
                // console.log(this.objInfo.families)
                if(action === "edit"){
                    // this.objInfo.families = row.families_selected
                    this.showFormulario=true
                }
                else
        ...
        // beforeCreate(){
        //     infomaster.families([],"select").then(data => {
        //         console.log(data.data)
        //             this.tableFamily =JSON.parse(JSON.stringify( data.data))
        //         })
        // },
        created(){
