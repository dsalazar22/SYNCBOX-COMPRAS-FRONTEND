<template>
    <div>
        ...
        <h4 class="font-weight-bold py-3 mb-4">Ubicaciones</h4>
                ...
                <b-table small show-empty stacked="md" :items="tableData" :fields="columns" :current-page="currentPage" :per-page="perPage" 
                 ...
                 <div class="d-flex justify-content-between">
                    ...
                    <div class="text-muted">
                        <small> Total Ubicaciones: {{this.totalRows}} </small>
                    </div>
                </div>
            </div>
        </div>
        ...
        <b-modal id="modals-top" v-model="showFormulario"  class="modal-center" size="md">
            ...
            Administrar <span class="font-weight-light">Ubicaciones</span><br>
            ...
            <fieldset class="demo-vertical-spacing-sm">
                ...
                <b-form-group :feedback="objState.message">
                    <b-input placeholder="Codigo de Barras" v-model.trim="objInfo.barcode" :state="objState.barcode!=''" v-on:input="objState.barcode = 'valid'; objState.barcode= objInfo.barcode==''?'invalid':'valid'"/>
                </b-form-group>
                ...
                <b-form-group :feedback="objState.message">
                     <label class="switcher switcher-success mt-2">
                        <input type="checkbox" class="switcher-input" v-model="objInfo.is_mobile">
                        <span class="switcher-indicator">
                        <span class="switcher-yes"><span class="ion ion-md-checkmark"></span></span>
                        <span class="switcher-no"><span class="ion ion-md-close"></span></span>
                        </span>
                        <span class="switcher-label">Es Movil</span>
                    </label>
                </b-form-group>
                <b-form-group :feedback="objState.message">
                    <label class="switcher switcher-success mt-2">
                        <input type="checkbox" class="switcher-input" v-model="objInfo.is_output">
                        <span class="switcher-indicator">
                        <span class="switcher-yes"><span class="ion ion-md-checkmark"></span></span>
                        <span class="switcher-no"><span class="ion ion-md-close"></span></span>
                        </span>
                        <span class="switcher-label">Es Ubicacion de Salida</span>
                    </label>
                </b-form-group>
            </fieldset>
    ...
    export default {
        ...
        data: () => ({
            ...
            /// Columnas de las tablas
                columns: [
                    ...
                    {key:'is_mobile', label:"Es Movil"},
                    // {key:'is_close', label:"Cerrado"},
                    // {key:'is_output', label:"Salida"},
        ...
        ///Eventos CRUD
            submit: function(){
                if(this.validate_content()){
                    ...
                    this.objInfo.warehouse_id = this.selectedWarehouse.warehouse_id
                    this.objInfo.is_output = this.objInfo.is_output == undefined ? false : this.objInfo.is_output
                    this.objInfo.is_mobile = this.objInfo.is_mobile == undefined ? false : this.objInfo.is_mobile
                    
                    infoinventory.ubications(this.objInfo, this.objInfo.ubications_id === 0 ? "add" : "update").then(data => {
                        if (data.status === 202){
                            this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                        }else{
                            if(this.objInfo.ubications_id === 0)
            ...
            deleteRecord: function(){
                infoinventory.ubications(this.objInfo, "del").then(data => {
                    if (data.status === 202){
        ...
        ///Preracion de datos para el borrado y el modificado desde el evento de la tabla
            data_change :function(row,action){
                ...
                if(action === "edit"){
                    this.showFormulario=true
                    this.objInfo.ubications_id = row.ubications_id
                    this.selectedWarehouse = {}
                    for (let index = 0; index < this.warehouse.length; index++) {
                        const element = this.warehouse[index];
                        if(element.warehouse_id == row.warehouse_id){
