<template>
    <div>
        ...
        <h4 class="font-weight-bold py-3 mb-4">Bodega</h4>
                ...
                <b-table small show-empty stacked="md" :items="tableData" :fields="columns" :current-page="currentPage" :per-page="perPage" 
                 ...
                 <div class="d-flex justify-content-between">
                    ...
                    <div class="text-muted">
                        <small> Total Bodegas: {{this.totalRows}} </small>
                    </div>
                </div>
            </div>
        </div>
        ...
        <b-modal id="modals-top" v-model="showFormulario"  class="modal-center" size="md">
            ...
            Administrar <span class="font-weight-light">Bodegas</span><br>
            ...
            <fieldset class="demo-vertical-spacing-sm">
                <b-form-group :feedback="objState.message">
                    <b-input placeholder="Código Bodega" v-model.trim="objInfo.code" :state="objState.code!=''" v-on:input="objState.code = 'valid'; objState.code= objInfo.code==''?'invalid':'valid'"/>
                </b-form-group>
                <b-form-group :feedback="objState.message">
                    <b-input placeholder="Nombre Bodega" v-model.trim="objInfo.name" :state="objState.name!=''" v-on:input="objState.name = 'valid'; objState.name= objInfo.name==''?'invalid':'valid'"/>
                </b-form-group>
                <b-form-group :feedback="objState.message">
                    <label class="switcher switcher-success mt-2">
                        <input type="checkbox" class="switcher-input" v-model="objInfo.include_available">
                        <span class="switcher-indicator">
                        <span class="switcher-yes"><span class="ion ion-md-checkmark"></span></span>
                        <span class="switcher-no"><span class="ion ion-md-close"></span></span>
                        </span>
                        <span class="switcher-label">Incluir en el Cálculo del Inventario Disponible</span>
                    </label>
                </b-form-group>    
            </fieldset>
    ...
    export default {
        ...
        data: () => ({
            ...
            /// Columnas de las tablas
                columns: [
                    {key:'code', label:"Código Bodega"},
                    {key:'name', label:"Nombre Bodega"},
        ...
        ///Eventos CRUD
            submit: function(){
                if(this.validate_content()){
                    infoinventory.warehouse(this.objInfo, this.objInfo.warehouse_id === 0 ? "add" : "update").then(data => {
                        if (data.status === 202){
                            this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                        }else{
                            if(this.objInfo.warehouse_id === 0)
            ...
            deleteRecord: function(){
                infoinventory.warehouse(this.objInfo, "del").then(data => {
                    if (data.status === 202){
        ...
        ///Preracion de datos para el borrado y el modificado desde el evento de la tabla
            data_change :function(row,action){
                ...
                if(action === "edit"){
                    this.showFormulario=true
                    this.objInfo.warehouse_id = row.warehouse_id
                }
                else
