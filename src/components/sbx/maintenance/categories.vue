<template>
    <div>
        ...
        <h4 class="font-weight-bold py-3 mb-4">{{titulo}}</h4>
            ...
            <!-- OPCIONES DE BUSQUEDA Y BOTONES PARA CREAR NUEVOS REGISTROS -->
                ...
                <b-table small show-empty stacked="md" :items="tableData" :fields="columns" :current-page="currentPage" :per-page="perPage" 
                 ...
                 <div class="d-flex justify-content-between">
                   ...
                   <small> Total Categorias de Activos: {{this.totalRows}} </small>
                    </div>
                    </div>
            </div>
        </div>
        ...
        <b-modal id="modals-top" v-model="showFormulario"  class="modal-center" size="md">
            ...
            Administrar <span class="font-weight-light">Categorías de Activos</span><br>
    ...
    export default {
        ...
        ///Eventos CRUD
            submit: function(){
                ...
                infomainten.categories(this.objInfo, this.objInfo.categories_id === 0 ? "insert" : "update").then(data => {
                    if (data.status === 202){
                        this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                    }else{
                        if(this.objInfo.categories_id === 0)
            ...
            deleteRecord: function(){
                infomainten.categories(this.objInfo, "delete").then(data => {
                    if (data.status === 202){
