<template>
    <div>
        ...
        <h4 class="font-weight-bold py-3 mb-4">{{$route.params.module == "prd" ? "Familia de Productos" : "Grupos de Partes"}}</h4>
            ...
            <!-- OPCIONES DE BUSQUEDA Y BOTONES PARA CREAR NUEVOS REGISTROS -->
                ...
                <b-table small show-empty stacked="md" :items="tableData" :fields="columns" :current-page="currentPage" :per-page="perPage" 
                 ...
                 <div class="d-flex justify-content-between">
                   ...
                   <small> Total Familia de Productos: {{this.totalRows}} </small>
                    </div>
                    </div>
            </div>
        </div>
        ...
        <b-modal id="modals-top" v-model="showFormulario"  class="modal-center" size="md">
            ...
            Administrar <span class="font-weight-light">{{$route.params.module == "prd" ? "Familia de Productos" : "Grupos de Partes"}}</span><br>
            ...
            <fieldset class="demo-vertical-spacing-sm">
                ...
                <b-form-group label="Días de Buffer" :feedback="objState.message">
                    <b-input placeholder="Buffer" :disabled="disabledcode" v-model.trim="objInfo.buffer" />
                </b-form-group>
                <div>
                    <div class="row">
    ...
    export default {
        ...
        ///Eventos CRUD
            submit: function(){
                ...
                infomaster.families(this.objInfo, this.objInfo.families_id === 0 ? "insert" : "update").then(data => {
                    if (data.status === 202){
                        this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                    }else{
                        if(this.objInfo.families_id === 0)
            ...
            deleteRecord: function(){
                infomaster.families(this.objInfo, "delete").then(data => {
                    if (data.status === 202){
            ...
            showData: function(){
                infomaster.families(this.selectedModule,"select","").then(data => {
