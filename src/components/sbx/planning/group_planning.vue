    ...
    <div>
        ...
        <div class="row">
            ...
            <div class="col">
                    ...
                    <div class="row" v-if="selectTab == 'producto'">
                        <div class="col">
                            <div class="position-relative">
                                ...
                                <h5 class="mt-2">Cantidad a Producir</h5>
                                <b-input-group>
                ...
                <b-table small :fields="columnsParentsProducts"  :items="parentsProducts" class="mt-2">
...
export default {
    ...
    methods: {
        ...
        async controllerAmount(){
            ...
            await infoplanning.explosionCode({code: this.selectedProductItem.code, amount: this.product_ppal.amount.toString()}).then(data =>{
                this.preparedBOM = []
                this.preparedListasMateriales = []
                if(data.data != ""){

                    for (let index = 0; index < this.parentsProducts.length; index++) {
                        const element = this.parentsProducts[index];
                        if(element.Code == this.selectedProductItem.code)
                            this.parentsProducts.splice(index, 1); 
                    }

                    this.parentsProducts.push({
                        Code: this.selectedProductItem.code,
                        Description: this.selectedProductItem.description,
                        RequestedAmount: this.product_ppal.amount,
                        Sugerido: 0,
                        totales: data.data[0]["totales"],
                        materiales: data.data[0]["materiales"],
                    })
                }
            })
        ...
        totalProducts(){
            ...
            for (let i = 0; i < this.parentsProducts.length; i++) {
                const e = this.parentsProducts[i];

                for (let j = 0; j < e.totales.length; j++) {
                    const m = e.totales[j];
                    let pos = -1
                    for (let k = 0; k < this.preparedBOM.length; k++) {
                        const n = this.preparedBOM[k];
                        if(m.Code==n.Code){
                            pos = k
                        }
                    }
                    ...
                    if (pos == -1) {
                        this.preparedBOM.push({
                            Code: m.Code,
                            Description: m.Description,
                            RequestedAmount: m.RequestedAmount,
                            InventoryAmount: m.InventoryAmount,
                        })
                    }else{
                        this.preparedBOM[j].RequestedAmount = this.preparedBOM[j].RequestedAmount+m.RequestedAmount
                        pos = -1
                    }
                }
            }
