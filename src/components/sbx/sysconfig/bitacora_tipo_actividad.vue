<template>
    <div>
        ...
        <h4 class="font-weight-bold py-3 mb-4">Bitácora Tipos de Actividades</h4>
        ...
        <div class="row">
            ...
            <b-row>
                ...
                <b-col md="2" class="d-flex justify-content-center my-1">
                    <b-btn variant="outline-success icon-btn" class="btn-md" v-if="check_access('admin_tools_type', 'c')"  v-b-modal.modals-default @click.prevent="addData(); "><i class="ion ion-md-add"></i></b-btn>&nbsp;
            ...
            <!-- OPCIONES DE BUSQUEDA Y BOTONES PARA CREAR NUEVOS REGISTROS -->
                ...
                <b-table small show-empty stacked="md" :items="tableData" :fields="columns" :current-page="currentPage" :per-page="perPage" 
                 ...
                 <div class="d-flex justify-content-between">
                   ...
                   <small> Total Tipos de Actividades: {{this.totalRows}} </small>
                    </div>
                    </div>
            </div>
        </div>
        ...
        <b-modal id="modals-top" v-model="showFormulario"  class="modal-center" size="md">
            ...
            Administrar <span class="font-weight-light">Tipos de Actividades</span><br>
...
<script>
    ...
    export default {
        ...
        data: () => ({
            ...
            modified:false,
            tableData: [],            
            arrModulo:[{ value: 'mnt', text: 'Mantenimiento' },{ value: 'prd', text: 'Producción' }, { value: 'qual', text: 'Calidad' },  { value: 'gral', text: 'General' }],
        ...
        methods:{
            getArea(area){
                for (let index = 0; index < this.arrModulo.length; index++) {
                    const element = this.arrModulo[index];

                    if(element.value == area){
                        return element.text
                    }
        ...
        ///Eventos CRUD
            submit: function(){
                if(this.validate_content()){
                    infosysconfig.btctypes(this.objInfo, this.objInfo.btc_types_id === 0 ? "insert" : "update").then(data => {
                        if (data.status === 202){
                            this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                        }else{
                            if(this.objInfo.btc_types_id === 0)
            ...
            deleteRecord: function(){
                infosysconfig.btctypes(this.objInfo, "delete").then(data => {
                    if (data.status === 202){
        ...
        ///Preracion de datos para el borrado y el modificado desde el evento de la tabla
            data_change :function(row,action){
                ...
                if(action === "edit"){
                    this.showFormulario=true
                    this.objInfo.btc_types_id = row.btc_types_id
                }
                else
