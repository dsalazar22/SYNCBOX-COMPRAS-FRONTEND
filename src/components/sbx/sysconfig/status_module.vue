<template>
    <div>
        ...
        <h4 class="font-weight-bold py-3 mb-4">Estado de los Documentos</h4>
        <div class="w-100 text-right"></div>
        
        <b-form-group >
            <b-form-radio-group v-model="document_selected" :options="options" name="radioInline"  @input="test"/>
        </b-form-group>
        ...
        <div class="row">
            ...
            <b-row>
                ...
                <b-col md="2" class="d-flex justify-content-center my-1">
                    <b-btn variant="outline-success icon-btn" class="btn-md" v-if="check_access('admin_document_status', 'c') && document_selected != ''" v-b-modal.modals-default @click.prevent="addData();objState={}" ><i class="ion ion-md-add"></i></b-btn>&nbsp;
            ...
            <!-- OPCIONES DE BUSQUEDA Y BOTONES PARA CREAR NUEVOS REGISTROS -->
                ...
                <b-table small show-empty stacked="md" :items="tableData" :fields="columns" :current-page="currentPage" :per-page="perPage" 
                    ...
                    <template v-slot:cell(edit)="row" v-if="document_selected!='production_process'">
                        <div class="text-center">
                            <b-btn variant="outline-success borderless icon-btn" class="btn-xs" v-if="check_access('admin_document_status', 'c')" @click.prevent="data_change(row.item,'edit')"><i class="ion ion-md-create"></i></b-btn>
                ...
                <small> Total Documentos: {{this.totalRows}} </small>
                 </div>
                    </div>
            </div>
        </div>
        ...
        <div class="d-inline-block">
            <sweet-modal icon="error" title="Borrar registro" ref="errorWithButtonAlert">
                ...
                <template slot="button"><b-btn @click="deleteRecord()" v-if="check_access('admin_document_status', 'd')"  variant="danger" class="mr-2">Eliminar</b-btn></template>
        ...
        <b-modal id="modals-top" v-model="showFormulario"  class="modal-center" size="md">
            ...
            Administrar <span class="font-weight-light">Estado de los Documentos</span><br>
            ...
            <fieldset class="demo-vertical-spacing-sm">
                ...
                <b-form-group :feedback="objState.message">
                 ...
                 <label class="switcher switcher-success mt-2">
                    ...
                    <span class="switcher-label">Al activar esta opción, generará una equivalencia a que este tipo de documento está aún activo, generando impacto en los inventarios, disponibles, ordenes de producción, entre otros</span>
                </label>
            </fieldset>
            <div slot="modal-footer" class="w-100 text-right">
                <b-btn variant="primary" @click="cancel()">Cancelar</b-btn>
                <b-btn :variant="variantSaveButton" v-if="check_access('admin_document_status', 'c')"  @click="submit()">Guardar</b-btn>
            </div>
        </b-modal>
...
<script>
    ...
    export default {
        ...
        data: () => ({
            ...
            options: [
                { text: 'Ordenes de Producción', value: 'production_order' },
                { text: 'Pedidos', value: 'orders' },
                { text: 'Compras', value: 'purchases' },
                { text: 'Intervalos', value: 'intervals' },
                { text: 'Proceso de Producción', value: 'production_process' },
                { text: 'Mantenimiento', value: 'maintenance' }
            ],
            ...
            /// Columnas de las tablas
                columns: [
                    ...
                    {key:'is_active', label:"Documento Activo - Disponible"},
        ...
        ///Eventos CRUD
            submit: function(){
                if(this.validate_content()){
                    this.objInfo.module = this.document_selected
                    infosysconfig.documentstatus(this.objInfo, this.objInfo.status_id === undefined ? "insert" : "update").then(data => {
                        if (data.status === 202){
                            this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                        }else{
                            if(this.objInfo.status_id === undefined)
            ...
            deleteRecord: function(){
                infosysconfig.documentstatus(this.objInfo, "delete").then(data => {
                    if (data.status === 202){
            ...
            showData: function(){
                this.objInfo.module = this.document_selected
                infosysconfig.documentstatus(this.objInfo, "select").then(data => {
