<template>
    <div>
        ...
        <h4 class="font-weight-bold py-3 mb-4">Tipos de Herramentales</h4>
        ...
        <div class="row">
            ...
            <b-row>
                ...
                <b-col md="2" class="d-flex justify-content-center my-1">
                    <b-btn variant="outline-success icon-btn" class="btn-md" v-if="check_access('admin_tools_type', 'c')"  v-b-modal.modals-default @click.prevent="addData(); objState={}"><i class="ion ion-md-add"></i></b-btn>&nbsp;
            ...
            <!-- OPCIONES DE BUSQUEDA Y BOTONES PARA CREAR NUEVOS REGISTROS -->
                ...
                <b-table small show-empty stacked="md" :items="tableData" :fields="columns" :current-page="currentPage" :per-page="perPage" 
                    ...
                    <template v-slot:cell(edit)="row">
                        <div>
                            <b-btn variant="outline-success borderless icon-btn" class="btn-xs" v-if="check_access('admin_tools_type', 'c')" @click.prevent="data_change(row.item,'edit')"><i class="ion ion-md-create"></i></b-btn>
                    ...
                    <template v-slot:cell(is_tool)="row">
                        <div>
                            <!-- <i class="ion ion-md-create"></i> -->
                        <span style="color:red" v-if="!row.item.is_tool"><i class="ion ion-md-remove-circle"></i></span>
                            <span style="color:green" v-if="row.item.is_tool"><i class="ion ion-md-add-circle"></i></span>
                        </div>
                    </template>
                 ...
                 <div class="d-flex justify-content-between">
                   ...
                   <small> Total Tipos de Herramentales: {{this.totalRows}} </small>
                    </div>
                    </div>
            </div>
        </div>
        ...
        <div class="d-inline-block">
            <sweet-modal icon="error" title="Borrar registro" ref="errorWithButtonAlert">
                ...
                <template slot="button"><b-btn v-if="check_access('admin_tools_type', 'd')"  @click="deleteRecord()" variant="danger" class="mr-2">Eliminar</b-btn></template>
        ...
        <b-modal id="modals-top" v-model="showFormulario"  class="modal-center" size="md">
            ...
            Administrar <span class="font-weight-light">Tipos de Herramentales</span><br>
            ...
            <fieldset class="demo-vertical-spacing-sm">
                ...
                <label class="switcher switcher-success mt-2">
                    <input type="checkbox" class="switcher-input" v-model="objInfo.is_tool">
                    <span class="switcher-indicator">
                    <span class="switcher-yes"><span class="ion ion-md-checkmark"></span></span>
                    <span class="switcher-no"><span class="ion ion-md-close"></span></span>
                    </span>
                    <span class="switcher-label">Es Herramental</span>
                </label>
            </fieldset>
            <div slot="modal-footer" class="w-100 text-right">
                <b-btn variant="primary" @click="cancel()">Cancelar</b-btn>
                <b-btn :variant="variantSaveButton" v-if="check_access('admin_tools_type', 'c')"  @click="submit()">Guardar</b-btn>
            </div>
        </b-modal>
    ...
    export default {
        ...
        data: () => ({
            ...
            /// Columnas de las tablas
                columns: [
                    ...
                    {key:'is_tool', label:"Es Herramental"},
        ...
        ///Eventos CRUD
            submit: function(){
                if(this.validate_content()){
                    infosysconfig.toolstype(this.objInfo, this.objInfo.tool_type_id === 0 ? "insert" : "update").then(data => {
                        if (data.status === 202){
                            this.showCustom('bg-danger text-white', "Error","¡Error Guardando el Registro!")
                        }else{
                            if(this.objInfo.tool_type_id === 0)
            ...
            deleteRecord: function(){
                infosysconfig.toolstype(this.objInfo, "delete").then(data => {
                    if (data.status === 202){
        ...
        ///Preracion de datos para el borrado y el modificado desde el evento de la tabla
            data_change :function(row,action){
                ...
                if(action === "edit"){
                    this.showFormulario=true
                    this.objInfo.tool_type_id = row.tool_type_id
                }
                else
