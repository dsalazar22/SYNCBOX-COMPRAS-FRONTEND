<template>
    ...
    <sidenav orientation="horizontal" :showDropdownOnHover="false" class="bg-white layout-navbar align-items-lg-center container-p-x" style="position: relative;z-index: 99">
      <div class="sidenav-inner">
        ...
        -->
        <sidenav-menu icon="far fa-id-badge">
          <template slot="link-text">Comercial</template>
            ...
            <sidenav-router-link class="ml-0" v-if="check_access('new_order_comm', 'r')" to="/comm/rel" :exact="true">Liberar Pedidos</sidenav-router-link>
        </sidenav-menu>
        <sidenav-menu icon="fas fa-wallet">
          <template slot="link-text">Cartera</template>
            <sidenav-router-link class="ml-0" v-if="check_access('new_quot_comm', 'r')" to="/comm/appr" :exact="true">Aprobar Pedidos</sidenav-router-link>
            <sidenav-router-link class="ml-0" v-if="check_access('liberar_comm', 'r')" to="/comm/cart" :exact="true">Cartera</sidenav-router-link>
        </sidenav-menu>


        <sidenav-router-link class="ml-0" v-if="check_access('new_order_comm', 'r')" to="/comm/newrq/local/0" icon="fas fa-plus" :exact="true">Nuevo Pedido</sidenav-router-link>
        <sidenav-router-link class="ml-0" v-if="check_access('bill_comm', 'r')" to="/comm/cot/local/0" icon="fas fa-plus" :exact="true">Nueva Cotizacion</sidenav-router-link>
